...
# 1. Added 'upper' to the imports list here
from pyspark.sql.functions import col, to_timestamp, when, current_timestamp, lit, upper

# 1. Initialize local Spark with Scala 2.13 and Postgres/Delta dependencies (Aligned to 4.3.0)
spark = SparkSession.builder \
    .appName("Local_Silver_Cleaning") \
    .config("spark.jars.packages", "org.postgresql:postgresql:42.6.0,io.delta:delta-spark_2.13:4.3.0") \
...
df_bronze = spark.read.format("delta").load("./storage/bronze/orders")
...
df_conformed = df_bronze \
    ...
    .withColumn("total_amount", col("amount").cast("decimal(10,2)")) \
    .withColumn("order_status", when(col("status").isNull(), "UNKNOWN").otherwise(upper(col("status")))) \
...
# 6. Write both datasets to their respective Delta tables (Updated paths)
...
if df_quarantine.count() > 0:
    ...
    df_quarantine.write \
        .format("delta") \
        .mode("append") \
        .save("./storage/quarantine/orders")
